...
export default function AppsPage() {
  return <><ThemeToggle /><header className="navbar"><div className="container nav-inner"><a className="brand" href="/" aria-label="上海树下小屋网络科技有限公司"><span className="brand-icon" aria-hidden="true">✦</span><span className="brand-text"><span className="cn">上海树下小屋网络科技有限公司</span><span className="en">Products · Support · Privacy</span></span></a><nav className="nav-menu nav-menu-static"><a className="nav-link" href="/">首页</a><a className="nav-link" href="/support/">客户服务</a><a className="nav-link cta-nav" href="/contact/">联系我们</a></nav></div></header><main className="product-page"><div className="container product-page-inner"><section className="product-hero apps-hero"><p className="route-eyebrow">sxxw.site · 应用支持中心</p><h1>每一款应用，都有清楚的说明与可控的数据边界。</h1><p>在这里找到产品介绍、新手引导、隐私政策、用户协议与技术支持。请按你实际使用的平台阅读相应内容。</p></section><section className="app-directory" aria-label="应用列表">{apps.map((app, index) => <article className="app-directory-card" key={app.name}><span className="app-index">0{index + 1}</span><div className="app-main"><p>{app.platform}</p><h2>{app.name}</h2><span>{app.description}</span></div><div className="app-actions"><em>{app.tag}</em><a className="btn-ghost" href={app.href}>查看应用</a></div></article>)}</section></div></main><footer className="site-footer"><div className="container footer-inner"><div className="footer-left"><div className="footer-brand">上海树下小屋网络科技有限公司</div><div className="footer-slogan">产品介绍 · 新手引导 · 隐私与支持</div></div></div></footer></>;
}
